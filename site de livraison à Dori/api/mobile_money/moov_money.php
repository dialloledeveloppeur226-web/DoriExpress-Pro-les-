...
/**
 * =============================================
 * API MOOV MONEY - DoriExpress-Pro
 * =============================================
 * Fichier : api/mobile_money/moov_money.php
 * Rôle : Intégration Moov Money Burkina Faso
 * Niveau : Premium
 * =============================================
 */
 ...
 */
function moov_money_create_payment($commande_id, $montant, $telephone) {
    $api_key = get_parametre('moov_money_api_key', '');
    $merchant_id = get_parametre('moov_money_merchant_id', '');
    $api_url = get_parametre('moov_money_api_url', 'https://api.moov.africa/moov-money/');
    
    if (empty($api_key) || empty($merchant_id)) {
        return ['success' => false, 'message' => 'Moov Money non configuré'];
    }
    ...
    $response = [
        'success' => true,
        'transaction_id' => 'MV-' . date('YmdHis') . '-' . rand(1000, 9999),
 ...
 */
function moov_money_verify_payment($transaction_id) {
    try {
        $db = Database::getInstance();
...
if ($action === 'paiement' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = moov_money_create_payment($commande_id, $montant, $telephone);
...
if ($action === 'verifier' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $result = moov_money_verify_payment($transaction_id);
    echo json_encode($result);
    exit;
}
...
// =============================================
// FIN DU FICHIER API/MOBILE_MONEY/MOOV_MONEY.PHP
// =============================================
?>